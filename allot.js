document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateExamOptions() {
                ...
                // Update allotments list when date/time changes
            ...
            // Get the allotment details for a specific room
            function getRoomAllotmentDetails(roomNo, date, time) {
                if (!date || !time) return null;
                
                const matchingAllotment = allotments.find(allotment => 
                    allotment.date === date && 
                    allotment.time === time && 
                    allotment.allocation.some(a => a.roomNo === roomNo)
                );
                
                if (!matchingAllotment) return null;
                
                const roomAllocation = matchingAllotment.allocation.find(a => a.roomNo === roomNo);
                
                return {
                    subject: matchingAllotment.subject,
                    semester: matchingAllotment.semester,
                    branch: matchingAllotment.branch,
                    allocatedSeats: roomAllocation ? roomAllocation.allocatedSeats : 0
                };
            }
            ...
            function renderExamHalls(roomsToRender = rooms) {
                ...
                roomsToRender.forEach((room, index) => {
                    ...
                    const allotmentDetails = isAllotted ? getRoomAllotmentDetails(room.roomNo, selectedDate, selectedTime) : null;
                    ...
                    if (isAllotted && allotmentDetails) {
                        const tooltip = document.createElement('div');
                        tooltip.className = 'absolute hidden bg-red-600 text-white text-xs rounded py-1 px-2 -top-16 left-1/2 transform -translate-x-1/2 whitespace-nowrap z-10 w-40 text-center';
                        tooltip.innerHTML = `
                            <p>Already allotted</p>
                            <p>${allotmentDetails.subject}</p>
                            <p>${allotmentDetails.semester}-${allotmentDetails.branch}</p>
                            <p>${allotmentDetails.allocatedSeats} seats</p>
                        `;
                    ...
                    hallElement.addEventListener('click', () => {
                        ...
                        if (isAllotted) {
                            const details = allotmentDetails;
                            alert(`Room ${room.roomNo} is already allotted for ${details.subject} (${details.semester}-${details.branch}) with ${details.allocatedSeats} seats.`);
                            return;
                        }
            ...
            function updateAllotmentsList() {
                // Filter allotments based on the selected date and time
                const selectedDate = dateSelect.value;
                const selectedTime = timeSelect.value;
                
                const filteredAllotments = allotments.filter(allotment => 
                    (!selectedDate || allotment.date === selectedDate) && 
                    (!selectedTime || allotment.time === selectedTime)
                );
                ...
                recentAllotments.forEach(allotment => {
                    ...
                    allotmentItem.className = 'border-b border-gray-200 py-2 last:border-0 relative hover:bg-gray-100 cursor-pointer';
                ...
                if (filteredAllotments.length > 5) {
            ...
            function deleteAllotment(allotmentId) {
                ...
                if (index !== -1) {
                    ...
                    updateAllotmentsList();
                    updateStatistics();
                    renderExamHalls(); // Re-render exam halls to update allocation status
                ...
                if (filteredAllotments.length === 0) {
                    allotmentsModalContent.innerHTML = `
                        <div class="text-center py-8 text-gray-500">
                            ...
                            <p>No allotments available for the selected date and time.</p>
                        </div>
                    `;
                } else {
                    // Sort allotments by date (newest first)
